...
def add_illustrations_to_linreg(html_content, illustrations):
    ...
    # Add residuals visualization after metrics section
    residuals_pattern = r'(<h2>🔷 5\. Метрики качества.*?</table>\s*</div>)'
...
def add_illustrations_to_polynomial(html_content, illustrations):
    ...
    if match:
        insert_pos = match.end(1)
        html_content = html_content[:insert_pos] + degrees_img + html_content[insert_pos:]
    
    # Add overfitting illustration after visualization example
    overfitting_pattern = r'(plt\.show\(\)</code></pre>\s*</div>\s*<div class="block">\s*<h2>🔷 8\. Переобучение)'
...
def add_illustrations_to_ridge_lasso(html_content, illustrations):
    ...
    # Add comparison of regularization methods after ElasticNet section
    comparison_pattern = r'(<h2>🔷 5\. ElasticNet.*?</code></pre>\s*</div>)'
    ...
    coef_pattern = r'(<h2>🔷 6\. Сравнение Ridge vs Lasso.*?</table>\s*</div>)'
    ...
    if match:
        insert_pos = match.end(1)
        html_content = html_content[:insert_pos] + coef_img + html_content[insert_pos:]
    
    # Add regularization path
    path_pattern = r'(<h2>🔷 7\. Подбор alpha.*?</code></pre>\s*</div>)'
